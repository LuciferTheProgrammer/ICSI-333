...
void displayWorld(char *masterWord, int descriptor, int validateGuess) {
    ...
    char *template1 = "<html><body style=\"background-color: black; color: white;\"><h1 style=\"border: 4px solid yellow; color: orange; font-size: 24px; text-align: center;\"> Words Without Friends Game</h1>"
                    "<form action=\"words\" method=\"GET\"><div> Enter a guess: " 
   ...
   if(validateGuess == -1) {
        char *invalid = "<p style=\"text-align: right; color: red; padding: 10px;\">"
                        "Invalid Guess</p>";
    ...
    sprintf(buffer + strlen(buffer), "<p style=\"border: 4px solid gray; color: blue; font-size: 20px; font-style: italic; text-align: center;\">");
    for(int i = 0; i < strlen(placement); i++)
        sprintf(buffer + strlen(buffer), "%c ", placement[i]);
    sprintf(buffer + strlen(buffer), "</p>");
    sprintf(buffer + strlen(buffer), "<p style=\"color: gray;\">------------------------------------------------------------</p>");
    ...
    while(took != NULL) {
        if(took->hasBeenFound)
            sprintf(buffer + strlen(buffer), "<p style=\"background-color: green; color: white; font-size: 16px;\">FOUND: %s</p>", took->stringForGame);
        else {
            char toContain[1000];
            memset(toContain, '\0', sizeof(toContain));
            char *stringPlace = printUnderScores(took->stringForGame, toContain);
            sprintf(buffer + strlen(buffer), "<p style=\"color: red; font-size: 16px;\">%s</p>", stringPlace);
        }
        took = took->next;
    }
...
void *processClientRequest(void *pass) {
    ...
    if(strstr(holderPointer, "words")) {
        ...
        if(!isDone())
            ...
            cleanupGameListNodes();
            char *body = "<html><body style=\"background-color: blue; color: white;\"<h1 style=\"font-size: 32px; text-align: center; color: yellow;\">Congratulations! You solved it! <a href=\"words\">Another?</a></h1></body></html>";
